...
static HHOOK hook;
static DWORD key = VK_LMENU;
static void loadSettings();
static LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam);
static void initKeyboardHook();
static void cleanupKeyboardHook();
...
static LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
  if (nCode == HC_ACTION)
  {
    ...
    if ((wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) && pKeyBoard->vkCode == key)
    {
      char path[MAX_PATH];
      GetModuleFileNameA(NULL, path, MAX_PATH);
      for (int i = (int)strlen(path) - 1; i >= 0; i--)
      ...
      {
        uiStopCb(NULL);
        strcat(path, "disable.wav");
        PlaySound(path, NULL, SND_FILENAME | SND_ASYNC);
      }
      else
      {
        uiStartCb(NULL);
        strcat(path, "enable.wav");
        PlaySound(path, NULL, SND_FILENAME | SND_ASYNC);
      }
    }
  }
